...
const COMMANDS : [&str; 9] = ["add_entity", "remove_entity", "add_effect", "remove_effect", "damage", "heal", "clear", "save", "load"];
...
impl eframe::App for Content {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let text_field = egui::TextEdit::singleline(&mut self.text).desired_width(f32::INFINITY).lock_focus(true);
                    ...
                    Some(word) => {
                        ...
                        for name in matchables.iter() {
                            if name.to_lowercase().starts_with(&word.to_lowercase()) {
                                ...
                                new_text = Some(self.text[..self.text.len() - word.len()].to_string() + name + " ");
                            }
                        }
                        if let Some(new_text) = new_text {
                            ...
                            if let Some(mut state) = egui::TextEdit::load_state(ui.ctx(), text_edit_id) {
                                let ccursor = egui::text::CCursor::new(self.text.chars().count()-1);
